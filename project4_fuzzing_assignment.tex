...
\begin{itemize}
    ...
    \item a fuzz harness and intentionally seeded memory-safety bugs,
    \item helper scripts for building, fuzzing, reproducing crashes, and post-fix regression testing, and
    \item a \texttt{student\_targets/} directory with starter templates for Part 2.
\end{itemize}
...
Use the project package provided on Canvas. If you receive a zip archive, extract it and enter the repository root. If you were instead given a Git repository link, you may clone it.
...
You need:
\begin{itemize}
    \item Git (only if using the Git repository distribution)
...
\begin{itemize}
    ...
    \item preserves normal behavior on the provided valid PNG inputs, as checked by \texttt{./scripts/test\_valid.sh}.
\end{itemize}
...
\subsection*{Suggested Subject Domains}
You do \textbf{not} have to use these, but these are good kinds of programs to build targets around:
\begin{itemize}
    \item small data structure implementations,
    \item language parsers, formatters, or pretty-printers,
    \item string normalization or cleanup routines,
    \item encode/decode or serialize/deserialize helpers,
    \item small arithmetic or numeric helper libraries,
    \item tiny standalone open-source utilities or packages.
\end{itemize}

\subsection*{Suggested Property Ideas}
You do \textbf{not} have to use these exact properties, but these are the right level of ambition:
\begin{itemize}
...
Part 2 is intended to be lighter-weight than Part 1, and the default expectation is that you will compile your Part 2 targets locally with a libFuzzer-capable \texttt{clang++}.
...
On some systems (especially macOS), the default \texttt{clang++} may not include a working libFuzzer runtime. If you see errors mentioning missing \texttt{libclang\_rt.fuzzer}, you may need to install a version of LLVM/Clang with libFuzzer support.
...
\begin{itemize}
    ...
    \item clarifying how fuzzing, libFuzzer, or toolchain setup operates,
...
\appendix
\section*{Appendix: Expected Output Examples}
...
\subsection*{Expected fuzzing behavior}
