...
This script:
\begin{itemize}
    \item builds the OSS-Fuzz image,
...
This runs the fuzzer for 30 seconds. You may increase the time if needed.
...
For generated crash inputs, replace the argument with the appropriate file path.
...
\begin{itemize}
    ...
    \item a small folder named \texttt{reproducers/} containing two files, one input for each bug you fixed.
\end{itemize}
...
These reproducer files are raw binary inputs. They are \textbf{not} screenshots or pasted text. They should be the exact files you use with \texttt{./scripts/reproduce.sh}.
